...
const COMMIT_HYGIENE_PROMPT = `# Commit & PR Authorship
...
- NEVER add \`Co-Authored-By:\` trailers naming yourself, the model, opencode, Claude, GPT, or any AI tool.
- NEVER add "Generated with", "Created with", "Written with", "via <tool>", "🤖", or any other AI-attribution footer, signature, or watermark.
- NEVER add marketing links (e.g. https://opencode.ai, https://claude.com/claude-code, https://anthropic.com) to commit messages, PR bodies, issue comments, or any other artifact.
  ...
  Effect.gen(function* () {
    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const llmTitle = yield* Effect.gen(function* () {
        ...
        return cleaned ? (cleaned.length > 100 ? cleaned.substring(0, 97) + "..." : cleaned) : undefined
      }).pipe(Effect.catch(() => Effect.succeed(undefined as string | undefined)))

      const t = llmTitle ?? fallbackTitle(firstUser)
      if (!t) return
      yield* sessions
      ...
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            system.push(COMMIT_HYGIENE_PROMPT)
